...
public class AsistenciaServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("AsistenciaServlet POST - "
            + "Acción: " + accion + ", Rol: " + rol);

        // VALIDACIÓN DE PERMISOS POR ROL
        if (!validarAccesoRol(rol, accion)) {
            System.out.println("ACCESO DENEGADO POST: Rol " + rol + 
            " intentó acceder con acción: " + accion);
            response.sendRedirect("acceso_denegado.jsp");
            return;
        }
    ...
    private void verificarLimiteEdicion(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String mensaje = configuracionDAO.obtenerMensajeLimite(cursoId, 
                    turnoId, fecha, horaClase);            
            // Responder en JSON
            ...
            System.out.println(" Verificación de límite: puedeEditar=" 
                    + puedeEditar);            
        } catch (Exception e) {
            System.out.println("Error al verificar límite: " + e.getMessage());            e.printStackTrace();
            e.printStackTrace();
    ...
    private void verAsistenciasCursoJson(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("Obteniendo asistencias "
                    + "JSON para curso: " + cursoId + ", fecha: " + fecha);
            List<Asistencia> asistencias = asistenciaDAO.obtenerAsistenciasPorCursoTurnoFecha(cursoId, turnoId, fecha);
    ...
    private void verAsistenciasPadre(HttpServletRequest request, HttpServletResponse response)
        ...
        if (padre == null) {
            session.setAttribute("error", "Sesión expirada. Por favor inicie sesión nuevamente.");            response.sendRedirect("index.jsp");
            return;
        }
            ...
            if (alumnoIdParam != null) {
                try {
                    ...
                    if (paramAlumnoId != alumnoId) {
                        System.out.println("⚠️ ADVERTENCIA: Padre intentó "
                                + "acceder a asistencias de alumno ID: " +  paramAlumnoId + " pero su hijo es ID: " + alumnoId);
                    }
                } catch (NumberFormatException e) {
        ...
        try {
            ...
            if (cursoSeleccionado != null && fechaParam != null && !fechaParam.isEmpty() && 
                ...
                try {
                    ...
                    System.out.println("Error al verificar límite: " + e.getMessage());                    // Si hay error, permitir edición por defecto
                    puedeEditar = true;
